  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <button onClick={() => setShowUpload(true)} className="bg-accent text-white text-xs font-black px-4 py-2.5">
          + 영상 소재 업로드
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowUpload(false)}>
          <div className="bg-white p-8 max-w-md w-full rounded-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleUpload} disabled={uploading} className="flex-1 bg-accent text-white text-[11px] font-black py-3">
                {uploading ? '업로드 중...' : '업로드'}
              </button>
            </div>
          </div>
